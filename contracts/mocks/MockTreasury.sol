...
contract MockRenaissanceTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public sART;
  uint256 public sARTQueue; // Delays change to sART address
  ...
  constructor(
    ...
    // isLiquidityToken[ _ARTDAI ] = true;
    // liquidityTokens.push( _ARTDAI );
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sART).balanceOf(msg.sender); // Can only borrow against sART held
  ...
  /**
        ...
        @param _amount uint
     */
  function repayDebtWithART(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");
    ...
    returns (uint256 value_)
  {
    if (isReserveToken[_token]) {
      ...
      value_ = _amount.mul(10**IERC20(ART).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SART) {
      // 9

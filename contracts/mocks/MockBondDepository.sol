...
contract MockRenaissanceBondDepository is Ownable {
  ...
  struct Bond {
    uint256 payout; // ART remaining to be paid
  ...
  function deposit(
    ...
    require(payout >= 10000000, "Bond too small"); // must be > 0.01 ART ( underflow protection )
    ...
    /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) ART
         */
    ...
    if (fee != 0) {
      ...
      IERC20(ART).safeTransfer(DAO, fee);
    }
  ...
  function stakeOrSend(
    ...
    if (!_stake) {
      ...
      IERC20(ART).transfer(_recipient, _amount); // send payout
    } else {
      // if user wants to stake
      if (useHelper) {
        // use if staking warmup is 0
        IERC20(ART).approve(stakingHelper, _amount);
  ...
  function maxPayout() public view returns (uint256) {
    return IERC20(ART).totalSupply().mul(terms.maxPayout).div(100000);
  }
  ...
  /**
   *  @notice calculate current ratio of debt to ART supply
   *  @return debtRatio_ uint
   */
  function debtRatio() public view returns (uint256 debtRatio_) {
    uint256 supply = IERC20(ART).totalSupply();
  ...
  /**
   *  @notice calculate amount of ART available for claim by depositor
   *  @param _depositor address
  ...
  /**
   *  @notice allow anyone to send lost tokens (excluding principle or ART) to the DAO
   *  @return bool
   */
